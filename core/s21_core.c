...
// to normalize decimal by exponent // нужно для сложения вычитания, не нужно
// для биг децималов
...
void signHandler(s21_decimal value_1, s21_decimal value_2,
                 s21_decimal *result) {
...
int expHandler(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  int RESULT_CODE = SUCCESS;
  int new_exponent =
      getExponent(value_1) + getExponent(*result) - getExponent(value_2);
...
int old_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  clearDecimal(result);
  int RESULT_CODE = SUCCESS;
  ...
  if (isEqualZero(value_2)) {
    ...
    RESULT_CODE = s21_div(value_1, value_2, &tmp_res);
    if (RESULT_CODE) {
    ...
    if (!RESULT_CODE) {
      ...
      s21_mul(value_2, tmp_res, &tmp_res);
